...
They range from chimpanzee to zebrafish (Figure 1). The whole-genome
...
non-syntenic fills between human and other species (Fig. 1). As
...
tend to have more but smaller fills (mean length ≤ 1 kb, Fig. 1A,B).
...
thresholds, 10 kb, 100 kb, and 1000 kb, respectively (Fig. 1C).
...
cattle, opossum, and zebrafish (Fig. 2B) at the 10 kb size threshold.
...
breakpoints at domain boundaries (Fig. 2C). For virtually all species
and size thresholds analyzed, we found breakpoints significantly
enriched at boundaries of TADs and contact domains (Fig. 2C, S1).
Depletion was only observed for some combinations of species and size
thresholds which have only very few breakpoints (see Fig. 1C).
...
for actual breakpoints compared to random controls (Fig. S2). Overall,
...
GRBs (Fig. 3A). This is consistent with previous studies in *Drosophila*
...
TADs for GRB-TADs compared to non-GRB-TADs (Fig. 3B). However, also the
...
the FANTOM5 project (Table. S1) [@Forrest2014]. If a human gene and its mouse
...
unique one-to-one ortholog in mouse (Table S2), 1,525 have a
...
0.0015, Fig. 4A). This indicates higher conservation of gene regulation
...
(Fig. 4B). This shows that disruptions of TADs by evolutionary
...
compared to 8,038 genes in non-GRB TADs (Fig. 4C, p = 0.13).
...
intervening TAD boundaries [@Ibn-Salem2014; @Lupianez2015; @Spielmann2012]. This would explain the selective
...
identifiers listed in Table 1 and S1.
