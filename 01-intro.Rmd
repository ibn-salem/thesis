...
Thereby, patterns of open chromatin are identified using DNase-seq [@Song2010] or ATAC-seq [@Buenrostro2013]. Active enhancers show short bidirectionl transcription, which can be identifed using CAGE [@Andersson2014]. Enhancer associate with specific patterns of posttranslational histone modifications, such as H3K27ac and H3K4me1. These epigentic marks, as well as TF binding events, can be detected genome-wide by chromatin immunoprecipitation coupled with sequencing (ChIP-seq) experiments [@Creyghton2010].

Enhancers often regulate multiple genes and genes are regulated by multiple enhancers.
...
physical proximity during cross-linking but originate from different regions in the linear genome sequence [@Dekker2013; @Andrey2017]. 
...
and is reviewed comprehensively [@Pombo2015; @Sexton2015; @Bouwman2015; @Dekker2016; @Dixon2016; @Schmitt2016; @Bonev2016; @Hnisz2016a; @Merkenschlager2016; @Long2016; @Rowley2016; @Ruiz-Velasco2017; @Andrey2017].

The genome is thought to be hierarchically organised on different genomic-length scales (Fig. \@ref(fig:GenomeHierarchy)). 
Whole chromosomes occupy distinct terriotories in the nucleus. Thereby,
...
These domains are made up from individual looping interactions and restrict regultory contacts between enhancers and gene promoters.
These heararchical levels are described bellow in more detail.
...
(ref:GenomeHierarchy) **Hierarchical organization of three-dimensional chromatin architecture** The left side shows a high-resolution Hi-C interaction map as a heat-map. The color intensity represents the interaction frequencies of orthogonally associated loci. The right side shows models of the individual genome structures. **(A)** Interactions occur mostly between loci on the same chromosome, indicating chromosome territories. **(B)** Alternating patterns of interaction clusters show the association of chromatin into A/B compartments. **(C)** Triangles within enrichment for intra-TAD interactions. Borders of TADs insulate interactions between neighboring domains. **(D)** Peaks in the Hi-C heatmap indicate pairwise chromatin looping interactions of the associated loci. Several types of loops are shown on the right side. Figure adapted from [@Bonev2016]. 
...
revealed that individual regions on chromosomes segregate by preferential interactions into two major clusters, referred to as A/B-compartments (Fig. \@ref(fig:GenomeHierarchy)B) [@Lieberman-Aiden2009].
...
with bin resolution of 1 Mb. In 2012 higher resolution Hi-C maps of up to 40 kb lead to the identification of genomic regions with preferential interactions within them (Fig. \@ref(fig:GenomeHierarchy)C). 
...
number of upstream versus downstream interactions of this bin. Using hidden Markov models, TAD boundaries where then identified in regions where DI changes drastically [@Dixon2012]. 
...
Furthermore, domain organization in TADs was confirmed by a recently developed genome architecture mapping (GAM) appraoch, which relies on DNA sequencing from a collection of thin nuclear sections [@Beagrie2017].
...
The dynamics of chromatin architecture were also studied in response to stimuli, such as hormone signaling by progestin or estradiol. Despite large changes in the transcriptional activity, only small differences were observed in the domain organization of chromatin [@LeDily2014]. 
...
In summary, TAD like structures have been detected in many organisms, including files, worms, fungi, and bacteria, but variations in size and internal structure indicate differences in their formaiton and biological functions [@Dekker2015].
...
The insulating function of TADs, which restric promoter-enhancer interactions [@Symmons2014], suggests that alterations of TAD structures may
...
Interestingly, these initial results are largely confirmed by more recent studies investigating structural variants that disrupt TADs and lead to ectopic gene expression in genetic diseases [@Lupianez2015; @Franke2016; @Redin2017] and cancers [@Northcott2014; @Hnisz2016; @Weischenfeldt2016]. This is discussed in more detail bellow (Chapter \@ref(thesis-discussion)).
