...
Another layer in gene regulation is the three-dimensonal foldig structure of chromatin in the nucleus. 
...
Alltogether, there is accumulating evidence that TADs are fundamental units of chromosome organization [@Dixon2016]
...
TADs in form cluster by their epi-genomic type into A/B compartments and coalescence of compartments in the same chromosome forms chromosome territories [@Bonev2016]. 

## Dynamics of chromatin structure
It is important to keep in mind how chromosome folding change during cell division. Spatial organization is geerally studied in non-synchronous cells, of which interphase cells make up the biggest proportion [@Bouwman2015].
In interphase, chromosomes are decondenced and hierarchically orgnized into the territories, compartemtns, and TADs as decribed above. 
To prepare for cell division chromosomes untangel and condense, while transcritpion ceases almost entirely. Miotic chromosomes do not show prefferential organization, such as compartments or TAS [@Naumova2013]. Enhancer-promoter looping might absenct as well [@Dekker2014].
After cell division, chromosomes decondense and fold into the interphase hierarchy. While individual genes are realtively mobile during early G1 phase, they become quickly constrained to a small nuclear subvolume, after which genome fodling is relatively stable for the rest of the interphase [@Shubb2002; @Walter2003].
...
- These dynamics during cell-cicle rais the question of how the patern of 3D organization is re-established with each cell division.

### Charmatin dynamics across cell types
...
However, chromatin dynamics specify distinct gene expression programmes and biological functions [@Bonev2016].
...
- Can TADs provide a regulatory environment for co-regulation of duplicated genes?
...
The followig four chapters adress and discuss these questions. 
...
We analyze whole-genome alignement between human and other vertebrate genomes to systematically
investigate whether TADs represent conserved building blocks of genomes
and whether rearrangements of TADs lead to altered gene expression
programs (Chapter \@ref(TAD-evolution)). 
