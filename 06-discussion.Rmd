...
## Evolutionary novelties by gene duplications and altered regulatory environments
<!-- Creation of new genes in shared regulatory environments -->

However, the analysis of paralog regulation with respect to three-dimensional genome folding allows further conclusions about the evolutionary history of genes and how new functionality by novel genes is most efficient created during evolution. 
...
We find evolutionary rearrangements between distal vertebrate species highly enriched at TAD boundaries and depleted within TADs (Chapter \@ref(TAD-evolution)).
...
This is supported by stonrg enrichment for non-coding conservation in TADs [@Harmston2017].
...
However, in most cases, these gene miss-expression might be detrimental for an organism, as was observed in genetic diseases [@Ibn-Salem2014; @Lupianez2015] and cancers (as discussed below).
...
## Gene miss-expression by altered TADs in disease {#TAD-disease}

Their conservation during evolution supports the functional importance of TADs.
However, if TADs are indeed crucial for proper gene regulation during development, one would expect that TAD disruptions are associated with disease and lead to phenotypes by miss-regulation of phenotypically relevant genes.
...
Importantly, these effects of rearrangements in disease genomes are consistent with the comparative genomic analysis finding selective pressure on TAD disruptions.

The causal relationship between TAD disruption, miss-expression and pathogenic phenotypes has been studied recently in more detail (revied for example in [@Krijger2016; @Achinger-Kawecka2017; @Yu2017; @Andrey2017]). 
...
Deletions of TAD boundaries can lead to ectopic activation of genes by enhancers in neighboring TADs (Figure \@ref(TadDisruptions)A). 
...
Depending on their size and positioning, also duplications can bring enhancers into contact with gene promoters, which were before separated by a TAD boundary (Fig. \@ref(TadDisruptions)D).
Duplications involving no regulatory elements and is contained entirely within a TAD (intra-TAD) have in general no major effect on genes in the same TAD, as enhancer-promoter contacts are mainly invariant of genomic distance within TADs [@Symmons2016]. 
...
Inversions of TAD boundaries can bring promoters under the control of enhancers, that were before in different TADs (Fig. \@ref(TadDisruptions)C).
...
## Integrating genome folding data can explain the pathomechanism structural variants
...
The HPO provides a standardized vocabulary of phenotypic abnormalities encountered in human disease and relations between them. Furthermore, phenotypes are associations to disease by mono-genic diseases [@Kohler2014]. 
...
Approaches, like 7C, allows predicting genome folding by taking into account both, the tissue specificity and potentially altered genomic sequence. 
However, the application on predicting folding in rearranged genomes needs to be demonstrated. 

## Condition specific genome folding
...
A/B compartment structure has been by FISH experiments [@Wang2016d]. TADs were also observed in single-cell Hi-C data [@Nagano2013].
...
Accordingly, TADs seems to emerge from cell population averages and do not exist as static structures in individual cells.
...
It is not clear how molecular interactions between proteins at the nanometer scale organize micron-long protein-bound DNA molecules.
TAD formation leads to insulating or facilitating interactions between specific genomic elements?
...
According to this model, loops and TADs are formed by active extrusion of chromatin through a loop-extruding factor. Thereby, barrier elements, such as CTCF can arrest the extrusion and lead to stable loops. Multiple further extrusion processes within such a loop can explain TAD like structures in Hi-C [@Fudenberg2018]. 

However, it is not clear which factors drive loop formation. 
...
Interestingly, we could confirm many known architectural proteins, such as cohesin, CTCF, and ZNF143. 
