...
associations to diseases accumulates over the last years, the non-coding
parts of the genome is only very recently annotated in large
collaborative efforts \citep{Dunham2012, Kundaje2015, Andersson2014}.
These efforts lead to an increased understanding of the regulatory
...
proteins \citep{Hoffman2015}. The cross-linked chromatin is than
...
that where in close physical proximity during cross-linking but
...
The three-dimensonal organization of genome folding was studied
extensively in recent years and is reviewd comprehensively by leading
expertes in the field
\citep{Pombo2015, Sexton2015, Bouwman2015, Dekker2016, Dixon2016, Schmitt2016, Bonev2016, Hnisz2016, Merkenschlager2016, Long2016, Ruiz-Velasco2017, Andrey2017}.
...
Togther, there exists only a few but specific inter-chromosomal contacts
and the genome is non-randomly organized in the nucleus by chromosomes
occupying distinct spatial territories.
...
preferential interactions into two mayor clusters, referred to as
...
\citep{Bonev2016}. More recnetly, higher resolution Hi-C maps further
subdevided A/B compartments into six sub-compartments with prefferntial
interacitons and associations to distinct chromatin features
\citep{Rao2014}.
...
contacts introduce variability \citep{Dali2017, Forcato2017}. Therefore,
the number and size of TADs varies between different studies, making it
difficult to compare TADs in different conditions and species across
studies.
...
\citep{Dixon2012} and arround 1,200 physical domains of arround 100 kb
in \emph{Drosophila} genomes \citep{Sexton2012}. Analysis of 1kb or 5kb
...
\citep{Dixon2012}. TADs correspond to regions of early and late
replication timing \citep{Pope2014, Dileep2015} and lamina associated
domains (LADs) \citep{Dixon2012}. Importantly, enhancer-promoter
interactions seems to be mostly constrained within TADs
\citep{Shen2012, Ghavi-helm2014, Symmons2014}
...
cohesin, and lead to the formation of TADs. TADs form cluster by their
epigenomic type into A/B compartments and coalescence of compartments in
the same chromosome forms chromosome territories \citep{Bonev2016}.
...
within TADs but did not alter TAD strucures completley
\citep{Seitan2013, Zuin2014}. One study report a significant increase in
interactions between differnt TADs after cohesin depletion
\citep{Sofueva2013}. Depletio of CTCF had a similar effect with
increased inter-domains interactions \citep{Seitan2013, Zuin2014}. These
result suggest a functinoal role of cohesin and CTCF in
...
average over millions of cells used in the experiments. Therefore,
identified contacts might not be present in each individual cells and
can be can be dynamic over short time-scales in each individual cell. It
is important to keep in mind how chromosome folding change during cell
...
\citep{Bouwman2015}. In interphase, chromosomes are decondenced and
...
relatively mobile during early G1 phase, they become quickly constrained
to a small nuclear sub-volume, after which genome folding is relatively
stable for the rest of the interphase \citep{Chubb2002, Walter2003}.
...
exit strongly affect chromatin three-dimensional organization. These are
...
A/B compartments and TADs seems to be largley unaffected in these
...
\tightlist
\item
  Add percentage of shared boundaries between cell types
...
evolutionary breakpoints at TAD boundaries and hihglights a major role
of conserved CTCF binding sites in facilitating conservation of TADs
...
rearrangements during evolution and how this is associated to conserved
or divergent transcriptional regulation between species.
...
adjacent TADs significantly increase and genes in neighboring TADs were
...
leads to the question whether disrutpion of TADs could also be and
mechansim of disease pathology. This hypothesis were initilay dirven by
studying the etiology of the Liebernberg syndrome, in which an
upper-limb malformation phenotype is casued by deletion in the vinicity
of \emph{PITX1} gene. \emph{PITX1} becomes therby accessible to an
enhancer with specific activity in lower-limb and was missexpressed in a
corresponding mouse model \citep{Spielmann2012}. An overlap of thes
deletion with later identifed TAD boundaries lead to the hypothesis of
an regulatory effect mechanism of boundary deletions referred to as
enhancer adoption \citep{Spielmann2013a}.

An association of TAD disrutpions with disease was first shown by
computational analysis integrating TAD structures, with tissue specific
enhancers, and large crhomosomal deletions associated with clinical
...
differntiate for each deletion case between two possible pathogenic
effect mechanism that best explains the phenotypes observed in the
patients. Many deletions could be explained by a gene dosage mechansim
and haploinsufficiency of genes located within the deletion. However,
about 12 percent of cases could be best explained by a TAD boundary
disruption and a specific combinations of tissue-specific enhancers and
...
frequent then expected by chance \citep{Ibn-Salem2014}. Therfore, this
study shows for the first time an association of TAD disrutpiton with
genetic diseases and highglight an regulatory effect mechansims of TAD
disruption that need to be further investigated and has to be considered
when interpreting the gentic variations in patient genomes.
...
haplosensitive genes. AMendelian disease related to mutations in the
first gene is associated with HP1, and aMendelian disease related to
...
to a mutation in the gene adjacent to the deletion. Figure adopted from
...
Intersingly, these initial results are largly confirmed by very recent
studies investigating structural variants that disrupt TADs and lead to
ectopic gene expression in genetic diseases
\citep{Lupianez2015, Franke2016, Redin2017} and cancers
\citep{Northcott2014, Hnisz2016, Weischenfeldt2016}. This is discussed
in more detial in secion \#ref(thesis-discussion).
...
still unclear how exactley the genome folds into TADs and what
...
divergence between species. The accumulating evidence of the important
gene regulatory function of TADs leads to the question if TADs play an
important role in genetic diseases \citep{Spielmann2016}. More
...
represent only structural units of the genome or also important
functional building blocks in which gene regulation is coordinated. By
...
the aim to facilitate three-dimensonal folding analyisis in diverse
...
\hypertarget{tad-disruption-as-a-new-mechanism-of-disease-pathogenesis.}{%
\section{TAD disruption as a new mechanism of disease
pathogenesis.}\label{tad-disruption-as-a-new-mechanism-of-disease-pathogenesis.}}
